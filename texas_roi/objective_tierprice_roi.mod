...
## OBJECTIVE ##
maximize revenue:
  (PNS + (sum {y in 1..Y} (sum {t in 1..T}  (p_plus*e[t,y]*T_u)/((1+i)^y) - (sum {m in winter_months} (a1*pi0[m,y] + a2*pi1[m,y] + a3*pi2[m,y] + a4*pi3[m,y] + a5*pi4[m,y])*z[y]) - (sum {m in summer_months} (a1s*pi0[m,y] + a2s*pi1[m,y] + a3s*pi2[m,y] + a4s*pi3[m,y] + a5s*pi4[m,y])*z[y]) - 
    (sum {t in 8640..T} g[t,y]*T_u*a1*z[y]))) - PV*inv_pv - B*inv_b)/(PV*inv_pv + B*inv_b);
